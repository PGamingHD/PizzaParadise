        ...
        try {
            ...
            interaction.editReply({
                    ...
                    embeds: [
                        ...
                        .addField(`Bot Latency`, `\`\`\`re\n[ ${Math.floor((Date.now() - interaction.createdTimestamp) - 2 * Math.floor(client.ws.ping))}ms ]\`\`\``, true)
                        .addField(`API Latency`, `\`\`\`re\n[ ${Math.floor(client.ws.ping)}ms ]\`\`\``, true).setTimestamp()
                        .addField(`Database Latency`, `\`\`\`re\n[ ${evaled}ms ]\`\`\``)
                        .setFooter(`Requested by ${interaction.member.nickname}`, `${interaction.member.displayAvatarURL()}`)
                    ],
