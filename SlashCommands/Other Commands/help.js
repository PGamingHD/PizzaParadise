    ...
    module.exports = {
        name: 'help',
        description: 'Require help with the bot? Use this command to get quick help!',
        options: [{
            name: 'page',
            description: 'Pick a help page to display help for!',
        ...
        run: async (client, interaction, args, con, prefix) => {
            ...
            if (!page) {
                ...
                let embed = new MessageEmbed()
                    .setColor(ee.color)
                    .setTitle(`PizzaParadise | Slash Help`)
                    .setDescription(`Use \`/help [page]\` to swap between the pages.`)
                    .addField(`PizzaParadise Tutorial`, `View the bot tutorial with the command \`/tutorial\` if you need some extra help!`)
            ...
            if (page) {
                if (![1, 2, 3, 4, 5].includes(page.value)) {
                    const buttonrow = new MessageActionRow()
                        .addComponents(
                            new MessageButton()
                            .setStyle(`LINK`)
