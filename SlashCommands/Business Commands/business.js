    ...
    module.exports = {
        name: 'business',
        description: 'Display information about your currently active business!',
        /** 
         * @param {Client} client 
        ...
        run: async (client, interaction, args, con) => {
            ...
            con.query(`SELECT * FROM business LEFT JOIN userprofile ON business.userId = userprofile.userId WHERE business.userId = '${interaction.user.id}' LIMIT 2;`, function (err, rows) {
                if (err) throw err;
                if (rows && rows.length) {
                    if (rows[0].businessLevel < 10) {
                        ...
                        let embed = new MessageEmbed()
                            ...
                            .addField(`Profile Level`, `🎚️ ${rows[0].businessLevel}\n\n**Percentage Bar - ${percentageText}**\n${bar}`)
                        ...
                        let embed = new MessageEmbed()
                            ...
                            .addField(`Business Age`, `⏳ ${remadefinal} days`)
                            .setFooter(`${rows[0].userRank}`, `${rows[0].userRankLink}`)
                            .addField(`Profile Level`, `🎚️ 10 (MAXED)\n\n**Percentage Bar - 100%**\n▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰`)
                        //.setFooter(`${rows[0].userRankLink}`)
                        //if (rows[0].businessLevel === 0) {
                        //    embed.addField(`Level`, `🎚️ *Join our **Support Server** to get level (.support)*`)
                        //} else {
