...
module.exports = {
    ...
    run: async (client, message, args, con, prefix) => {
        con.query(`SELECT * FROM business LEFT JOIN ownedrecipes ON business.userId = ownedrecipes.userId LEFT JOIN ingredientstock ON business.access = ingredientstock.access WHERE business.userId = '${message.author.id}'`, function (error, results, fields) {
            ...
            if (results && results.length) {
                if (!args[0]) {
                    return message.reply({
                        embeds: [
                            ...
                            .setColor(ee.color)
                            .setTitle(`❓ Shops ❓`)
                            .addField(`Ingredients Shop`, `\`${prefix}shop ingredient\``, true)
                            .addField(`Recipe Shop`, `\`${prefix}shop recipe\``, true)
                            .addField(`Miscellaneous Shop`, `\`${prefix}shop misc\``, true)
                            .setDescription(`Choose a shop type with \`${prefix}shop [type]\``)
                        ]
                    })
                }
                ...
                if (shoptype === 'recipe') {
                    let embed = new MessageEmbed()
                        ...
                        .setDescription(`Swap market pages with \`${prefix}shop [page]\`!`)
                        .setFooter(`Buy something with ${prefix}buy [ID]`)
